...
export function RevenueChart({ data }: { data: RevenueItem[] }) {
  const maxVal = Math.max(...data.map(d => d.revenue), ...data.map(d => d.target), 1);
          ...
          {data.map((d) => (
            <div key={d.month} className="flex-1 flex flex-col items-center gap-1.5">
              ...
              <div className="w-full relative">
                <div className="w-full rounded-t-md bg-primary/15" style={{ height: `${(d.target / maxVal) * 120}px` }} />
                <div className="w-full rounded-t-md bg-accent/80 absolute bottom-0" style={{ height: `${(d.revenue / maxVal) * 120}px` }} />
              </div>
